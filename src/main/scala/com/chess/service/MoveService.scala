...
class MoveService(moveValidator: MoveValidator, checkValidator: CheckValidator) {
  ...
  private def getMovingPiece(from: Position)(implicit gameState: GameState): Either[MoveError, Piece] =
    gameState.pieces
      .get(from)
      .map(_.asRight)
      .getOrElse(NoPieceToMoveFromThisPosition(from).asLeft)

  private def updateGameState(movingPiece: Piece, move: Move)(implicit gameState: GameState): Either[MoveError, GameState] = {
