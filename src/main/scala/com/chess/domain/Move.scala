...
case class Move(from: Position, to: Position) {
  def isDiagonalMove: Boolean =
    Math.abs(from.x - to.x) - Math.abs(from.y - to.y) == 0

  def isLinearMove: Boolean =
    !(from.x != to.x && from.y != to.y)

  def isOneStepMove: Boolean =
    Math.abs(from.x - to.x) <= 1 && Math.abs(from.y - to.y) <= 1

  def isTwoStepUpDownMove: Boolean =
    from.x == to.x && Math.abs(from.y - to.y) <= 2
...
trait MoveType
object MoveType {
  object Linear extends MoveType
  object Diagonal extends MoveType
  object KnightJump extends MoveType
  object Mixed extends MoveType
  object OneStepMixed extends MoveType
  object PawnMixed extends MoveType
}
